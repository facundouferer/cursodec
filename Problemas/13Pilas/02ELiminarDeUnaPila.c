#include <stdio.h>
#include <stdlib.h>

// Definición del nodo de la pila
struct Nodo {
    int dato;
    struct Nodo* siguiente;
};

// Función para insertar un elemento en la pila (push)
void push(struct Nodo** tope, int valor) {
    struct Nodo* nuevoNodo = (struct Nodo*)malloc(sizeof(struct Nodo));
    nuevoNodo->dato = valor;
    nuevoNodo->siguiente = *tope;
    *tope = nuevoNodo;
}

// Función para eliminar el elemento en la cima de la pila (pop)
void pop(struct Nodo** tope) {
    if (*tope == NULL) {
        printf("La pila está vacía, no se puede hacer pop.\n");
        return;
    }
    struct Nodo* aux = *tope;      // Guardamos el nodo tope actual
    *tope = (*tope)->siguiente;    // El nuevo tope es el siguiente nodo
    printf("Elemento eliminado: %d\n", aux->dato);
    free(aux);                     // Liberamos memoria del nodo eliminado
}

// Función para mostrar el contenido de la pila
void mostrarPila(struct Nodo* tope) {
    if (tope == NULL) {
        printf("La pila está vacía.\n");
        return;
    }
    printf("Contenido de la pila:\n");
    while (tope != NULL) {
        printf("%d\n", tope->dato);
        tope = tope->siguiente;
    }
}

int main() {
    struct Nodo* pila = NULL;
    int numero;

    // Insertamos tres números en la pila
    for (int i = 0; i < 3; i++) {
        printf("Ingrese un número para apilar: ");
        scanf("%d", &numero);
        push(&pila, numero);
    }

    // Mostramos la pila antes de eliminar
    mostrarPila(pila);

    // Eliminamos el elemento en la cima (pop)
    pop(&pila);

    // Mostramos la pila después de eliminar el tope
    mostrarPila(pila);

    return 0;
}
